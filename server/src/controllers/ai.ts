...
export async function analyzeJob(req: Request, res: Response): Promise<void> {
  ...
  const requestHash = hashJobDescription(input);
  ...
  try {
    response = await anthropic.messages.create({
      ...
      system:
        ...
        'Return at most 5 skill gaps and 6 interview questions. Base everything ' +
        'strictly on the job description provided.',
      messages: [
        {
          role: 'user',
